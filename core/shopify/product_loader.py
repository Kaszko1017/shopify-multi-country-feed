"""Parses Shopify bulk JSONL into per-country variant records."""
import json
import logging
...
def clean_description(raw_description):
    ...
    for delimiter in ["Colour description", "Material & Care", "Size Guide", "Reviews"]:
...
class ProductLoader:
    """Reconstructs variant rows with country-level inventory from bulk JSONL."""

    def load_products_from_bulk_jsonl(
        self,
        jsonl_file_path: str,
        active_countries: Dict,
        location_country_map: Dict,
    ) -> List[Dict]:
        logger.info(f"Processing bulk JSONL: {jsonl_file_path}")
    ...
    def _build_country_variants_from_bulk(
        self,
        products: Dict,
        variants: Dict,
        ...
        logger.info("Building country variants...")
        country_inventory = {}
        ...
        for variant_id, levels in inventory_levels.items():
            for level in levels:
                ...
                if available_qty > 0 and location_id in location_country_map:
                    ...
                    logger.warning(f"Location {location_id} not in mapping")
        ...
        target_countries = sorted(countries_with_inventory) if countries_with_inventory else []
        ...
        in_stock = sum(1 for v in final_variants if v.get("inventory_quantity", 0) > 0)
        logger.info(f"Created {len(final_variants)} variants ({in_stock} in stock)")
        return final_variants

    def _create_country_variant_from_bulk(
        self,
        variant_data: Dict,
        product_data: Dict,
        country_code: str,
        variant_gid: str,
        country_inventory: Dict,
        active_countries: Dict,
    ) -> Dict:
